...
const Reminder = ({ reminder, reminders, setReminders, handleDeleteClick }) => {
  ...
  return (
    <>
      <li
        className={`w-96 mx-auto relative rounded-md border-2 border-brown-light pl-2 py-2 flex my-2 ${
          reminder.completed ? "line-through opacity-60" : ""
        }`}
      >
        <input type="checkbox" onClick={handleComplete} aria-label="checkbox" />
        <p className="mx-4 w-18 flex items-center">{reminder.date}</p>
        <p className="w-56 text-left">{reminder.text}</p>
